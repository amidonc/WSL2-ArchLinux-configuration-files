...
# Common commands for task (taskwarrior), the CLI task manager (todo
# lists).
...
if [ -x /usr/bin/youtube-dl ]; then
    ...
    alias ytvid='youtube-dl --add-metadata --no-playlist --no-part --write-description --newline --prefer-free-formats -o "~/Videos/Youtube/%(title)s.%(ext)s" '
fi
